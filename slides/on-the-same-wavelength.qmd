...
::: {.rule-line}
Compute was not a delay here; it was a cap. What finished set what the paper could claim.
:::
...
Then the turn, clearly, because someone will check. Lancaster, not Oxford. And say the split —
two months running, four months waiting — because this room will recognise it.

Then the promise, spoken, not printed: "Three strands follow — words, brainwaves, speech. Two of
the three are CPU-only, and I'll be specific about which parts are AI and which are deliberately
not." The rest of the talk pays that off.
:::
...
- **The data.** One observation is one person, one word, one reaction time — **345,666** in
  Study 1.
...
## 12,000 core-hours, or 1,150 {data-menu-title="Algebra, not hardware"}

[Act 1 — Words]{.act}

::: cols-6040
::: {}
- **376,054 reaction times.** Redoing that design at full scale: about **12,000 core-hours** —
  500 days on one core.
...
- **Two more exact steps reach about 1,150** — power is a success count, so chunks add up; and a
  plainer fit for `simr`, another 1.4×.
:::
...
46 is an **analytic screen**, not a `simr` result. It agreed with `simr` on every effect about
whether a crossing exists, but read about 40% low where checked. **A bracket, not an answer.**
:::
:::
:::

::: {.rule-line}
Algebra, not hardware: 12,000 core-hours became 1,150, and the same algebra moved the sample
size.
:::
...
MUST SAY: "treat 46 as a bracket, not an answer." A screen that ran 40% low where it could be
checked cannot carry a forty-fold claim without the qualifier.
...
- **32,000 posterior draws per fit** — 32,000 plausible answers, not one estimate.
...
::: {.rule-line}
One choice drives every core-hour here: 728,352 single-trial rows, not 23,000 averages.
:::
...
::: {.rule-line}
192 GB to prepare the data, 96 GB to model it: memory, not cores, sizes these jobs.
:::
...
While the script is on screen, say what the walltime is not: five days is the ceiling, not the
runtime. A maximal model on this many trials samples slowly, and the `-O1` pin slows it further.
...
## Two thirds of the array died in the compiler {data-menu-title="Compiler ICE"}
...
| Build | Result |
|---|---|
| threading + `-O3` | [ICE after 19 s]{.warn} |
| threading + `-O2` | [ICE after 4 s]{.warn} |
| threading + `-O1` | builds in 90 s — **chosen** |
| no threading + `-O3` | builds in 82 s |
:::

::: {}
- **Peak memory under 2 GB** against a 96 GB allocation. **A compiler bug, not a resource
  limit.**
...
- **The leakage guard.** Leave one whole participant out of every fold, so no one person sits on
  both sides of a split. Chance is measured, not assumed: shuffle the labels within participant
...
`--array=0-2` — one task per property, 8 CPUs × 32 GB = **256 GB**, and the memory goes on
rebuilding the trial × channel × time tensor, not on the decoding.
...
This is a constraint on the *science*, not a preference. That framing is what makes the
GPU-on-ARC route the only one available.
...
the guide does not cover this, so the route has to be agreed rather than looked up.
...
- `openai/whisper-large-v3` on GPU, with `pyannote/speaker-diarization-3.1` labelling who spoke
  ...
  5 corpora × 16 text-cleanup conditions = 240 cells**.
:::
...
::: {.rule-line}
The GPU decodes each corpus once; a seventeenth condition costs no GPU time. Don't spend a GPU
allocation on string comparison.
:::
:::
:::
...
The economical part: sixteen text-cleanup conditions scored off one decoding pass, so they
cost one GPU pass, not sixteen.
...
Over 499 AMI meeting utterances, Whisper's starts are near frame-accurate; its **ends are not**.
...
::: {.redbox}
16 mini-meetings, not the full test set, and that average hides a spread from **5.6% to 76.7%**.
:::
:::
:::

::: {.rule-line}
Cheap re-runs change what you are willing to verify — and to publish.
:::

::: src
`timestamp_summary.json` versus `evaluation_vad_refine/` (one utterance of 500 skipped);
...
The GPUs here run other people's models as measuring instruments, not as things to train.
:::
...
Close on the framing line. I am not training models; I am using other people's as measuring
instruments.
:::
...
**Brainwaves.** Chance measured rather than assumed: 1,000 re-runs of the decoder for each
...
2,000 — **before** anyone is recruited.
:::
...
::: notes
[~65 s] The close the brief asks for: what became newly *possible*, not what got faster.

Open with the four bottlenecks, spoken rather than printed. Every one has already been shown:
memory in the 192 GB extraction, the compiler in the internal compiler error, the walltime in the
decoding wall, the filesystem in the weights staged out to nodes with no egress.
...
- **A documented route for special-category data.** The user guide covers storage and backups,
  not what may be held or who to agree it with. That conversation decides what social-science
  work I can bring here.
...
- **The array-size cap, written down.** `htc` capped one of my arrays at 1,000 tasks: 4,800
  ...
  instead of 300. The scheduling page documents partitions and walltimes, but not this.
:::
...
geometry and the *number* of fits, not dense linear algebra. The project's own rule says so, and
CI enforces it.
...
**Why `-O1` rather than dropping threading?** Because dropping threading changes `reduce_sum`'s
...
**Could the 14-day wall be shorter?** Fourteen days is what I asked for, not the partition's
ceiling. For the permutation null I would not go lower, but a fortnight backfills badly, and I
would rather checkpoint and request less than hold a reservation that size.
:::
:::
